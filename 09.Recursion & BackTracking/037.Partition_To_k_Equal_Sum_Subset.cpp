...
/*
  Time Complexity : O(N×2^N). At max 2^N
  unique bit masks are possible and during every recursive call,
  we iterate our original matchsticks array to sum up the values of matchsticks used to update the sides_formed variable.

   Space Complexity : O(N + 2^N) because N is the stack space taken up by recursion.
*/
